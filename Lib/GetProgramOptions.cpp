...
void GetProgramOptions::getOptions(int argc, char** argv, ProgramOptions &options)
	{
		po::variables_map vm;
		...
		try {
			...
			if (options.outputConfig.outputFormat != "csv" && options.outputConfig.outputFormat != "json")
				throw std::runtime_error("output format can only be internal type: json csv");
...
ProgramOptions GetProgramOptions::getFromFile(const std::string& filename)
{
...
std::pair<std::string, bool> GetProgramOptions::requieredVarCheck(const ProgramOptions& opt)
{
...
void GetProgramOptions::setDefaults(ProgramOptions& options)
{
